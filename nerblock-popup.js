...
chrome.tabs.query(
  ...
  function callback(tabs) {
    ...
    if (typeof bgpage.tabBlocks[currentTab.id] !== "undefined") {
      $("#page_blocked_count").html(
        bgpage.tabBlocks[currentTab.id].blocks.length
      );
    }
    $("#block_counts").show();
    $("#separator0").show();
        ...
        $("#div_tab_pause_nerblock")
          ...
          .click(function () {
            ...
            if (bgpage.tabBlocks[currentTab.id].blocks.length > 0) {
              for (
                var i = 0;
                i < bgpage.tabBlocks[currentTab.id].blocks.length;
                i++
              ) {
                if (
                  bgpage.tabBlocks[currentTab.id].blocks[i].type == "main_frame"
                ) {
...
function closePopOut() {
  var gettingInfo = chrome.runtime.getPlatformInfo(function gotPlatformInfo(
    info
  ) {
