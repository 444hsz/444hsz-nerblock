...
function reloadAllTabs() {
    var querying = chrome.tabs.query({}, function(tabs) {
        for (var i=0; i<tabs.length; i++) {
            var reloadurl = false;
            if (typeof tabBlocks[tabs[i].id] !== "undefined" && tabBlocks[tabs[i].id].blocks.length > 0) {
...
function setIconForTab(tabId, iconType, badge) {
    ...
    switch (iconType) {
        case "allow":
            iconBaseName = "nerblock-ok";
            break;
        default:
        case "deny":
            iconBaseName = "nerblock";
            break;
    }
    chrome.browserAction.setIcon({
        "path": {
            "19": "images/" + iconBaseName + "32.png",
            "20": "images/" + iconBaseName + "32.png",
            "25": "images/" + iconBaseName + "32.png",
            "30": "images/" + iconBaseName + "32.png",
            "35": "images/" + iconBaseName + "48.png",
            "38": "images/" + iconBaseName + "48.png",
            "40": "images/" + iconBaseName + "48.png"
        },
        "tabId": tabId
    });
    if (typeof badge !== "undefined") {
        chrome.browserAction.setBadgeBackgroundColor({"tabId": tabId, color: "#646464"});
...
chrome.webRequest.onBeforeRequest.addListener(function(details) {
    var isNERSite = false, url = (new URL(details.url)), keys = Object.keys(nerSites);

    if (
        globalPause
        || (url.protocol == "chrome-extension:" && url.pathname == "/nerblock-pageblocked.html") // dont run on extension pages
    ...
    for (var i=0; i<keys.length; i+=1) {
        if (nerSites[keys[i]].find(function(v) {
            ...
            else if (String(url.hostname + url.pathname.replace(/\/$/, "")) === v.replace(/^\*\./, "")) {
                //console.log("exact url match");
                return true;
            }
            else if (String(url.hostname + url.pathname.replace(/\/$/, "")).endsWith(v.replace(/^\*/, ""))) {
    ...
    if (details.tabId > -1) {
        if (details.type == "main_frame" || typeof tabBlocks[details.tabId] == "undefined") {
        ...
        if (typeof tabPauses[details.tabId] !== "undefined" && tabPauses[details.tabId] === true) {
            return {cancel: false};
        }
        ...
        if (details.type == "xmlhttprequest") {
            if (tabBlocks[details.tabId].blocks.length > 0) {
                setIconForTab(details.tabId, "deny", String(tabBlocks[details.tabId].blocks.length));
            }
        }
    }
    ...
    if (isNERSite) {
        console.log("Request to a website containing NER brainwashing propaganda has been intercepted: " + details.url + " (" + details.type + ")");
        if (!isEdge) {
            switch (details.type) {
                case "sub_frame":
                    ret = {redirectUrl: chrome.extension.getURL("nerblock-embedblocked.html?url=" + encodeURIComponent(details.url))};
                    break;
                case "main_frame":
                    ret = {redirectUrl: chrome.extension.getURL("nerblock-pageblocked.html?url=" + encodeURIComponent(details.url))};
                    break;
                default:
            }
        }
...
chrome.runtime.onConnect.addListener(function(p) {
    ...
    portFromCS.onMessage.addListener(function(m, s) {
        ...
        if (m.iframe) {
            ...
            setIconForTab(s.sender.tab.id, "deny", String(tabBlocks[s.sender.tab.id].blocks.length));
        }
    });
});  